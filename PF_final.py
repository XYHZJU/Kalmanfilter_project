...
if __name__ == "__main__":
    ...
    while (True):
        ...
        if match_counter==1:
            ...
            dx = (target_box_center[0] - X_posterior[0])[0]
            dy = (target_box_center[1] - X_posterior[1])[0]
            dx_list.append(dx)
            dy_list.append(dy)
            ...
            dx = 0.3*(dx_list[-1]) +  0.9*(dx_list[-2])
            dy = 0.3*(dy_list[-1]) +  0.9*(dy_list[-2])
            dx_list.append(dx)
            dy_list.append(dy)
        ...
        deltax = dx
        deltay = dy
        if(frame_counter>98 and frame_counter<102):
            deltax = -15   
        # print('v: ',frame_counter, dx, dy, target_box, X_posterior)
        ...
        cv2.putText(frame, "match count: "+str(match_counter), (25, 125), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(frame, "frame count: "+str(frame_counter), (25, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
